  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(tabName = "upload",
              p("Upload a count matrix and sample metadata. These files should be in 
                CSV format and match by sample IDs."),
              fileInput("count_file", "Upload Count Matrix", accept = ".csv"),
              fileInput("metadata_file", "Upload Metadata", accept = ".csv")
      ),
      
      tabItem(tabName = "de_tables",
              p("This table displays genes with significant differential expression based on your design matrix. 
                Sort, filter, or download the results below."),
      ...
      tabItem(tabName = "pca",
              fluidRow(
                box(title = "PCA Plot", status = "primary", solidHeader = TRUE, width = 12,
                    p("This plot visualizes global sample variation using Principal Component Analysis (PCA). 
         It helps identify clustering or separation between conditions or batches."),
                    plotlyOutput("pca_plot_interactive", height = "600px"),
                    br(),
                    downloadButton("download_pca", "Download PCA Plot", class = "btn-primary")
                )
              )
      ),
      ...
      tabItem(tabName = "volcano",
              fluidRow(
                box(title = "Volcano Plot", status = "primary", solidHeader = TRUE, width = 12,
                    p("This plot shows log2 fold-change vs. significance. Significant genes are easily 
                      spotted by their position in the upper corners."),
                    plotlyOutput("volcano_plot_interactive", height = "600px"),
                    br(),
                    downloadButton("download_volcano", "Download Volcano Plot", class = "btn-primary")
                )
              )
      ),
      ...
      tabItem(tabName = "heatmap",
              fluidRow(
                box(title = "Heatmap", status = "primary", solidHeader = TRUE, width = 12,
                    p("A clustered heatmap of the top differentially expressed genes. 
                      Rows = genes, Columns = samples."),
                    plotlyOutput("heatmap_plot", height = "700px"),
                    br(),
                    downloadButton("download_heatmap", "Download Heatmap", class = "btn-primary")
                )
              )
      ),
      ...
      tabItem(tabName = "kegg",
              fluidRow(
                box(title = "KEGG Enrichment Plot", status = "primary", solidHeader = TRUE, width = 12,
                    p("This tab shows pathway-level enrichment using KEGG. The dot plot visualizes 
                      enriched pathways, and a table provides full results."),
                    plotlyOutput("kegg_plot_interactive", height = "1000px")
                )
              ),
              fluidRow(
                box(title = "KEGG Enrichment Table", status = "primary", solidHeader = TRUE, width = 12,
                    DTOutput("kegg_table"),
                    br(),
                    downloadButton("download_kegg", "Download KEGG Results", class = "btn-primary")
                )
              )
      )
    )
